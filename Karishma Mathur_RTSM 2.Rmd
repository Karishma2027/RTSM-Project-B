...
# Augmented Dickey-Fuller (ADF) Test for Stationarity with Wipro Ltd on BSE Data
# ***********************
...
rel_ds = diff(log(rel_prices)); plot(rel_prices) #Wipro Ltd  (First)return 
...
lb_test_rel_ds = Box.test(rel_ds)# Inference : Wipro Difference (Stationary)
lb_test_rel_ds
...
acf(rel_prices) # ACF of WIPRO Series

pacf(rel_prices) # PACF of WIPRO Series

acf(rel_ds) # ACF of adani Series

pacf(rel_ds) # PACF of WIPRO Difference (Stationary) Series
...
wipro_returns <- diff(log(rel_prices))

# Remove missing values
wipro_returns <- na.omit(wipro_returns)

# Specify confidence level and time horizon
confidence_level <- 0.95
time_horizon <- 1  # 1 day

# Calculate VaR using historical method
VaR <- quantile(wipro_returns, 1 - confidence_level)
